...
**Criteria:** Given any span with a `parent_span_id` reference, there MUST exist a span with the same trace id that uses that value of `parent_span_id` as its `span_id` reference with.
